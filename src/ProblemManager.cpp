...
const std::vector<Problem> &ProblemManager::getAllProblems() const { return problems; }
...
std::vector<Problem> ProblemManager::searchAdvanced(const std::string &query) const
{
    auto rules = QueryParser::parse(query);
    std::vector<Problem> result;

    for (const auto &p : problems)
    {
        if (QueryParser::matches(p, rules))
        {
            result.push_back(p);
        }
    }

    if (result.empty() && rules.size() == 1 && rules[0].type == FilterRule::Type::NameMatches)
    {
        auto [index, distance, match] = fuzzy(rules[0].value, problems);
        if (index != -1 && distance <= getFuzzyThreshold(rules[0].value))
        {
            result.push_back(problems[index]);
        }
    }
...
bool ProblemManager::sortByTime()
{
    if ((int)problems.size() == 0)
        return false;
    std::sort(problems.begin(), problems.end(), [](const Problem &p1, const Problem &p2)
